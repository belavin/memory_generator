...
extern MemgenConfigStorage* mcs;
...
int QtclConsole::run_nc(ClientData client_data, Tcl_Interp* interp, int argc, const char *argv[]){
	Tcl_ResetResult(interp);


// Find ncverilog arguments (for calling from script)

QString nc_arguments;


for (int i=1; i <= argc; i++ ){
	nc_arguments = nc_arguments.append(" ");
	nc_arguments = nc_arguments.append(argv[i]);
}


// qDebug() << nc_arguments;
NcBatWriter* ncbat = new NcBatWriter( mcs,nc_arguments );
QString modifiedCommand = (QString)"exec /bin/chmod +x nc.bat;exec gnome-terminal -e ./nc.bat";
int res = Tcl_Eval( interp, qPrintable(modifiedCommand) );
...
int QtclConsole::run_rc(ClientData client_data, Tcl_Interp* interp, int argc, const char *argv[]){
	Tcl_ResetResult(interp);


// Find rtl compiler arguments (for calling from script)

QString rc_arguments;


for (int i=1; i <= argc; i++ ){
	rc_arguments = rc_arguments.append(" ");
	rc_arguments = rc_arguments.append(argv[i]);
}


qDebug() << rc_arguments;
RcBatWriter* rcbat = new RcBatWriter( mcs,rc_arguments );
QString modifiedCommand = (QString)"exec /bin/chmod +x rc.bat;exec gnome-terminal -e ./rc.bat";
int res = Tcl_Eval( interp, qPrintable(modifiedCommand) );
...
int QtclConsole::run_edi(ClientData client_data, Tcl_Interp* interp, int argc, const char *argv[]){
	Tcl_ResetResult(interp);


// Find rtl compiler arguments (for calling from script)

QString edi_arguments;


for (int i=1; i <= argc; i++ ){
	edi_arguments = edi_arguments.append(" ");
	edi_arguments = edi_arguments.append(argv[i]);
}


qDebug() << edi_arguments;
EncounterBatWriter* rcbat = new EncounterBatWriter( mcs,edi_arguments );
QString modifiedCommand = (QString)"exec /bin/chmod +x encounter.bat;exec gnome-terminal -e ./encounter.bat";
int res = Tcl_Eval( interp, qPrintable(modifiedCommand) );
...
int QtclConsole::loadScript( ClientData, Tcl_Interp* interp, int argc, const char *argv[])
{
		QString usageMsg = QString("Usage: %1 script_file_name\n").arg(argv[0]);
		// Reset result data
		Tcl_ResetResult(interp);
		...
		int res = Tcl_Eval( interp, qPrintable("source "+(QString)argv[1] ));

		//Get the string result of the executed command
		QString result = Tcl_GetString(Tcl_GetObjResult(interp));
		QConsole::interpretCommand(argv[1], &res);

/*		if (!QConsole::loadScript(argv[1]))
				return TCL_ERROR;
		else*/
				return TCL_OK;
}
...
int  QtclConsole::check_log(ClientData client_data, Tcl_Interp* interp, int argc, const char *argv[]){
	Tcl_ResetResult(interp);
	QString checklog_arguments;
...
//	int res = Tcl_Eval( interp, qPrintable(modifiedCommand) );
	...
	for (int i=1; i <= argc; i++ ){
				checklog_arguments = checklog_arguments.append(argv[i]);
	}
	QRegExp error_regexp(".*error.*");
	QString result = Tcl_GetString(Tcl_GetObjResult(interp));
	LogChecker* lgcheck = new LogChecker(checklog_arguments,error_regexp);
...
QtclConsole::QtclConsole(QWidget *parent, const QString &welcomeText) : QConsole(parent, welcomeText)
{
		...
		TclCallBack<QtclConsole>::registerMethod(this, (char*)"load_script",&QtclConsole::loadScript ,
							 (char*)"Load saved script");

//		TclCallBack<QtclConsole>::registerMethod(this, (char*)"run_encounter",&QtclConsole::saveScript ,
//					 (char*)"Run EDI with specified init file");


		TclCallBack<QtclConsole>::registerMethod(this, (char*)"run_nc",&QtclConsole::run_nc ,
							 (char*)"Run NcVerilog");

		TclCallBack<QtclConsole>::registerMethod(this, (char*)"check_log",&QtclConsole::check_log ,
									 (char*)"Check logfile for errors");


		TclCallBack<QtclConsole>::registerMethod(this, (char*)"run_rc",&QtclConsole::run_rc ,
							 (char*)"Run RTL Compiler ");

		TclCallBack<QtclConsole>::registerMethod(this, (char*)"run_edi",&QtclConsole::run_edi ,
							 (char*)"Run Encounter Digital Implementation");
